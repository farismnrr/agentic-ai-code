...
assert.ok(ingress.includes('error.message === \'Activity ingestion rejected\''), 'only source credential/identity rejection should map to HTTP 403')
assert.ok(database.includes('throw new Error(\'Activity ingestion invalid\')'), 'event validation failures must be distinguishable from source authorization failures')
assert.equal((database.match(/Activity ingestion rejected/g) ?? []).length, 2, 'only source token and source identity failures may use the revocation-class rejection')
...
const activityToolHelpers = read('packages/rust-tools/src/infrastructure/transport/tool_helpers.rs')
assert.ok(activityToolHelpers.includes('crate::core::redaction::redact_credentials'))
