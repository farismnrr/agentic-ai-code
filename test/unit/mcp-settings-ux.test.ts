...
const dialogFooter = read('app/components/settings/SettingsMcpConnectionDialogFooter.vue')
const dialogSurface = `${dialog}\n${dialogFooter}`
...
assert.match(dialogSurface, /label="Create"/)
assert.match(dialog, /discoverOAuth/)
assert.match(dialogSurface, /Scan tools/)
...
assert.match(dialogSurface, /form="mcp-connection-form"/)
