...
- **Bootstrap and resources:** `server/discover.instructions` (and the legacy-compatible `initialize.instructions`) advertise one bounded workspace/reporting bootstrap on every relay connection. The approved `workspace://<repo>/agent-guidance` resource includes `ai-self/BOOTSTRAP.md` when present, then repository `AGENTS.md` and `.agents/knowledge/resources.md`. The first-party Nuxt MCP path explicitly promotes discovered instructions into top-level chat and delegated-subagent system context only when provenance is `first-party-relay`; external/third-party MCP server instructions are never promoted by this mechanism. Because arbitrary external MCP hosts may ignore server instructions/resources, the relay also appends a compact reporting contract to each client-visible MCP wire tool description while leaving internal catalog definitions and tool-result contracts unchanged. Manual client bootstrap installation remains the final fallback when an external host ignores both server instructions and tool descriptions. Other bounded read-only resources expose repository manifest, Git status, and HEAD metadata; there are no arbitrary resource templates/subscriptions/file-browsing resources.
