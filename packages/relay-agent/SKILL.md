...
- **Tools:** Full exposes the immutable v15 retained catalog of 52 tools — local sandboxed execution (`terminal_exec`, `terminal_job_start`, `terminal_job_get`, `terminal_job_cancel`), configured network tools, Full-only read-only remote diagnostics (`ssh_readonly_exec`), bounded native workspace tools, remote Git transport, forge/issues/workflows, alerts, and Telegram integration. Primary is the intentional 15-tool core fast path. Local Git wrappers and LSP wrappers are not public catalog entries; use terminal for builds, tests, package managers, interpreters, scripts, and uncovered CLI work. The exact static catalog contract is frozen under `.agents/contracts/`.
...
- Local language-server workflows remain terminal fallback operations in v15. If a future catalog version promotes a reviewed semantic tool, its executable must be operator-approved through the existing bounded `--lsp-server language=executable` / `RELAY_LSP_SERVER` configuration and safe PATH checks.
...
Primary exposes 15 core terminal/workspace tools. Full exposes the 52-tool v15 retained catalog. Dedicated SSH diagnostics are Full-only and relay-owned: no client parses SSH config or receives raw SSH options/key paths. The catalog snapshot is versioned and immutable; agents receive only the selected active profile schemas and should prefer a supplied dedicated tool before terminal fallback.
