...
use relay_interfaces::mcp::{self, Response, Tool, ToolsCallParams};
...
pub(super) async fn try_handle_task_call(context: ToolCallContext<'_>) -> Option<JsonErr2> {
    ...
    if call.name == "terminal_job_start" {
        ...
        let task_id = match relay_application::execution::start_terminal_job(
            &call.arguments,
            &state.config,
            &state.jobs,
        )
    ...
    if execute_async {
        let tool_dispatch_started = Instant::now();
        let task_id = match relay_application::execution::start_tool_task(
            tool,
            &call.arguments,
            &state.config,
            &state.jobs,
