...
fn scaffold(
    ...
    let index = runtime::reviewed_index_html(&game, template);
    let script = runtime::reviewed_game_js(&game, template)?;
    let css = runtime::reviewed_css();
...
fn build_playtest(
    ...
    for role in &game.asset_roles {
        ...
        let target = format!("{build_root}/{}", role.runtime_path);
