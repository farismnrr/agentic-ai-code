...
fn git_log(arguments: &Value, config: &ServerConfig) -> Result<GitLogResult, McpError> {
    ...
    let mut owned = vec![
        ...
        format!(
            "--max-count={}",
            crate::application::continuation::MAX_TOTAL_ENTRIES + 1
        ),
    ];
    ...
    for rec in text
        ...
        .take(crate::application::continuation::MAX_TOTAL_ENTRIES + 1)
    {
    ...
    let truncated = commits.len() > crate::application::continuation::MAX_TOTAL_ENTRIES;
    commits.truncate(crate::application::continuation::MAX_TOTAL_ENTRIES);
    let root_scope = repo.root.to_string_lossy().into_owned();
    let (commits, continuation) = crate::application::continuation::paginate(
        arguments,
        commits,
        max,
        config,
...
fn append_protected_exclusions(args: &mut Vec<String>) {
    args.push("--".into());
    args.extend(crate::core::protected_paths::git_exclusion_pathspecs());
}
