...
pub fn directory_list(
    ...
    let directory = crate::core::workspace_path::resolve_existing_path_in_allowlist(
        &guard,
        cwd,
        requested_path,
        EntryKind::Directory,
    ...
    let mut state = TraversalState {
        ...
        max_entries: MAX_DIRECTORY_SCAN_ENTRIES
            .min(crate::application::continuation::MAX_TOTAL_ENTRIES),
        truncated: false,
    };
    visit_directory(&directory, Path::new(""), depth, &mut state)?;

    let (entries, continuation) = crate::application::continuation::paginate(
        arguments,
        state.entries,
        max_entries,
        config,
