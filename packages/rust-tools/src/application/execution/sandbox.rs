...
fn spawn_with_profile(
    ...
    let toolchain_mounts =
        toolchain_mounts::mount_toolchains(&mut args, config, sandbox_root, &host_home, control)?;
    let cargo_home = toolchain_mounts.cargo_home;
    let rustup_home = toolchain_mounts.rustup_home;
    let mut protected_path_freshness_checks = toolchain_mounts.freshness_checks;
