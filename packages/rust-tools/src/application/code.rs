...
//! against the existing LSP substrate (`crate::application::lsp`) and normalizes the
...
use crate::application::lsp::semantic::{Diagnostic, Hover, Location, RenameFilePreview, Symbol};
use crate::application::lsp::{
...
async fn code_symbols(
    ...
    match (path, query) {
        ...
        (None, Some(query)) => {
            let root = crate::application::git::resolve_git_workspace(cwd.as_deref(), config)
...
async fn code_references(
    ...
    let locations = adapter
        ...
        .map_err(lsp_error)?;
    let root = workspace_root(cwd.as_deref(), config)?;
    let (page, next) = crate::application::continuation::paginate(
        arguments,
        locations,
        DEFAULT_MAX_RESULTS,
        config,
...
fn workspace_root(cwd: Option<&str>, config: &ServerConfig) -> Result<PathBuf, McpError> {
    crate::application::git::resolve_git_workspace(cwd, config)
        .map_err(|_| McpError::InvalidRequest("cwd is not a usable workspace".into()))
}
