use ai_tools::infrastructure::telemetry::extract_ai_tools_traceparent;
...
enum Commands {
    ...
    /// Run the relay agent
    Relay(Box<ai_tools::core::config::Cli>),
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let telemetry_guard = ai_tools::infrastructure::telemetry::init_telemetry();
    ...
    ai_tools::infrastructure::telemetry::shutdown_telemetry_default(telemetry_guard).await;
