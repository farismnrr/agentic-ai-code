...
//!
//! `/mcp` is additionally gated by [`crate::infrastructure::security::enforce_local_access_policy`]
...
use crate::application::activity::SharedActivityRecorder;
use crate::application::admission::RequestAdmission;
use crate::core::config::ServerConfig;
use crate::core::error::McpError;
use crate::infrastructure::activity::ReloadableActivityRecorder;
use crate::infrastructure::auth;
use crate::infrastructure::auth::{CachedJwks, Claims};
use crate::infrastructure::notifications::TelegramMessageService;
use crate::infrastructure::observability::{CorrelationId, RequestId};
...
pub(super) use crate::interfaces::mcp::CODING_SCOPE;
...
pub struct AppState {
    ...
    pub jobs: std::sync::Arc<crate::application::execution::JobManager>,
    ...
    pub lsp: std::sync::Arc<crate::application::lsp::LspSessionManager>,
    /// Inert unless explicitly enabled and identity-validated at startup.
    pub hooks: std::sync::Arc<crate::application::hooks::HookManager>,
    ...
    /// Relay-owned explicit Telegram message queue. Credentials and destination remain
    /// fixed server-side and never come from MCP arguments.
    pub notifications: Arc<TelegramMessageService>,
}
...
pub fn create_router(config: ServerConfig) -> Router {
    let jobs = crate::application::execution::JobManager::new(config.clone());
    create_router_with_jobs(config, jobs)
}

pub fn create_router_with_jobs(
    config: ServerConfig,
    jobs: Arc<crate::application::execution::JobManager>,
) -> Router {
    let hooks = crate::application::hooks::HookManager::load(Arc::new(config.clone()))
...
pub fn create_router_with_jobs_and_hooks(
    ...
    let lsp =
        crate::application::lsp::LspSessionManager::new(config.clone()).unwrap_or_else(|_| {
            let mut unconfigured = config.clone();
            unconfigured.lsp_servers = Vec::new();
            crate::application::lsp::LspSessionManager::new(unconfigured)
                .expect("LSP session manager with no configured servers must construct")
        });
    let activity_control =
        crate::infrastructure::activity::ReloadableActivityRecorder::open(&config)
    ...
    let notifications = TelegramMessageService::open(&config)
        .expect("enabled Telegram message ledger must be available before relay startup");
