    ...
    tokio::spawn(async move {
        ...
        let (event, success, status) = match &waited {
            Ok(snapshot) => {
                let failed = !matches!(
                    snapshot.state,
                    crate::application::execution::JobState::Completed
                ) || snapshot
                    .result
                    .as_ref()
                ...
                if failed {
                    (
                        ...
                        if matches!(
                            snapshot.state,
                            crate::application::execution::JobState::Cancelled
                        ) {
                            Status::Cancelled
                        } else {
                            Status::Error
            ...
            snapshot
                ...
                .or_else(|| {
                    snapshot.map(|snapshot| {
                        crate::core::redaction::redact_credentials(&snapshot.output_text())
                    })
                })
        };
