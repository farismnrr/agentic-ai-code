...
use crate::interfaces::mcp::{self, Response, ToolCallResult};
...
pub(crate) fn activity_result_detail(
    ...
    let detail = detail
        ...
        .take(crate::application::activity::MAX_DETAIL)
        .collect::<String>();
...
pub(super) async fn finish_tool_call(context: ToolCompletionContext<'_>) -> JsonErr2 {
    ...
    let lifecycle_event = if result.is_error {
        crate::application::hooks::HookEvent::ToolError
    } else {
        crate::application::hooks::HookEvent::PostToolUse
    };
    let _ = state
        .hooks
        .invoke(
...
pub(super) async fn handle_agent_session_start(
    ...
    let (context, failure) = match outcome {
        crate::application::hooks::SessionStartOutcome::Started { context } => (context, None),
        crate::application::hooks::SessionStartOutcome::AlreadyStarted => (None, None),
