...
- `file_search(pattern, cwd?, max_results=100, exclude?)` — deterministic native glob discovery (`*`, `?`, and `**` path segments) with up to 16 exclusion globs; hidden files are searchable while `.git`, `node_modules`, `target`, `.nuxt`, and `.output` directories are skipped; hard result maximum 100.
