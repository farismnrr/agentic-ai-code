...
- **Database inspection requires least privilege.** Configure `RELAY_SSH_DB_READONLY_USER` and/or `RELAY_SSH_READONLY_REDIS_USER` for the relevant adapters. The relay combines a dedicated read-only identity with engine-level read-only/session controls, client hardening, semantic query/command validation, statement/output bounds, and denial of expensive or ambiguous operations. It never falls back to owner/superuser/migration credentials. A requested remote mutation must be performed manually by the operator; the relay returns a policy denial instead of attempting it.
- **Long-running / slow execution.** One bounded job manager remains the internal process-lifecycle authority for spawn, output draining, timeout, cancellation-on-request-drop, process-tree cleanup, retention, and concurrency. `terminal_exec` is synchronous-only and does not expose MCP Tasks or public terminal job polling. Independently task-capable non-terminal tools such as `ssh_readonly_exec`, `web_search`, and eligible read-like `http_fetch` calls retain their own advertised Tasks contracts.
- **Timeout policy.** `terminal_exec.timeout_ms` is caller-visible as `1..=60000` milliseconds with a 30-second default. Operator configuration may lower but never raise the 60-second terminal ceiling. Work expected to exceed it must be handed to the human operator as a foreground command without a timeout wrapper. HTTP client round-trip deadlines remain separate from durable task execution lifetime.
