...
pub(super) struct ForgeRepository {
    provider: &'static str,
    owner: String,
    repository: String,
}
...
pub(super) struct ChangeRequestSummary {
    number: u64,
    title: String,
    url: String,
    state: String,
...
pub(super) struct ChangeRequestListResult {
...
#[derive(Debug, Serialize)]
pub(super) struct ChangeRequestResult {
    repository_root: String,
    forge: ForgeRepository,
...
#[derive(Debug, Serialize)]
pub(super) struct ChangeRequestChecksResult {
    repository_root: String,
    forge: ForgeRepository,
    number: u64,
...
pub(super) async fn change_request_list(
    arguments: &Value,
    config: &ServerConfig,
...
pub(super) async fn change_request_get(
    arguments: &Value,
    config: &ServerConfig,
...
pub(super) async fn change_request_create(
    arguments: &Value,
    config: &ServerConfig,
) -> Result<ChangeRequestResult, McpError> {
    let repo = resolve_repo(arguments, config)?;
...
pub(super) async fn change_request_update(
    arguments: &Value,
    config: &ServerConfig,
) -> Result<ChangeRequestResult, McpError> {
    let repo = resolve_repo(arguments, config)?;
...
pub(super) async fn change_request_checks(
    arguments: &Value,
    config: &ServerConfig,
...
pub(super) async fn change_request_merge(
    arguments: &Value,
    config: &ServerConfig,
) -> Result<ChangeRequestResult, McpError> {
    let repo = resolve_repo(arguments, config)?;
...
fn requested_number(arguments: &Value) -> Result<u64, McpError> {
    arguments
        ...
        .ok_or_else(|| McpError::InvalidRequest("change request number is required".into()))
}
...
fn parse_json<T: serde::de::DeserializeOwned>(output: &[u8]) -> Result<T, McpError> {
    serde_json::from_slice(output)
        .map_err(|_| McpError::InvalidRequest("forge output is invalid".into()))
}

fn repo_spec(remote: &remote::GitRemoteIdentity) -> String {
    format!("{}/{}", remote.owner, remote.repository)
}

fn forge_identity(remote: &remote::GitRemoteIdentity) -> ForgeRepository {
    ForgeRepository {
