...
pub(in crate::git) struct WorkflowMutationResult {
...
#[derive(Debug, Serialize)]
pub(in crate::git) struct RunMutationResult {
    repository_root: String,
    forge: ForgeRepository,
    run_id: u64,
    operation: String,
...
pub(in crate::git) async fn workflow_dispatch(
    arguments: &Value,
    config: &ServerConfig,
...
pub(in crate::git) async fn workflow_run_rerun(
    arguments: &Value,
    config: &ServerConfig,
) -> Result<RunMutationResult, McpError> {
    run_mut(arguments, config, "rerun").await
}
pub(in crate::git) async fn workflow_run_cancel(
    arguments: &Value,
    config: &ServerConfig,
) -> Result<RunMutationResult, McpError> {
