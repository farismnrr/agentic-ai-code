...
fn runtime_fixture() -> (PathBuf, PathBuf, ServerConfig) {
    ...
    let external-mcp_auth = workspace.join("auth-external-mcp");
    let external-mcp_auth = workspace.join("auth-external-mcp");
    for path in [&workspace, &sibling, &bin, &external-mcp_auth, &external-mcp_auth] {
...
case "$mode" in
  ...
  sibling-test)
    ...
    ;;
  *)
    echo 'unsupported fake external-mcp invocation' >&2
    exit 2
    ;;
esac
"#,
...
"#,
    ...
    let config = ServerConfig {
        ...
        agent_auth_roots: vec![
            format!("external-mcp={}", external-mcp_auth.display()),
            format!("external-mcp={}", external-mcp_auth.display()),
        ],
...
async fn runtime_acceptance() {
    ...
    let timeout = run_agent(&config, &workspace, &["external-mcp"], "timeout-test", 100).await;
    ...
    let sibling = run_agent(&config, &workspace, &["external-mcp"], "sibling-test", 5_000).await;
