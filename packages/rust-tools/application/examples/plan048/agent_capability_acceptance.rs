...
fn main() {
    ...
    let codex = tool_catalog_for_profile_and_agent_providers(ToolProfile::Full, &["codex"]);
    let delegated = codex
        .iter()
    ...
    assert_eq!(
        AgentProvider::Codex.auth_probe_argv(),
        Some(["login", "status"].as_slice())
    );
    assert_eq!(
        AgentProvider::Claude.auth_probe_argv(),
