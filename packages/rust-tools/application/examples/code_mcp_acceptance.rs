...
use relay_application::code::dispatch_code_tool;
...
async fn run() -> Result<(), String> {
    for name in [
        ...
        require(
            relay_interfaces::mcp::find_tool(name).is_some(),
            &format!("{name} is present in the MCP catalog"),
        )?;
    }
    let rename_tool = relay_interfaces::mcp::find_tool("code_rename_preview").unwrap();
    require(
        rename_tool
            .annotations
            .as_ref()
