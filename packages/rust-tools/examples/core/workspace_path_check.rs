use ai_tools::core::workspace_path::{
...
fn expect_ok<T>(label: &str, result: Result<T, ai_tools::core::error::McpError>) -> T {
    match result {
        Ok(value) => value,
...
fn expect_err<T>(label: &str, result: Result<T, ai_tools::core::error::McpError>) {
    let _ = expect_error(label, result);
}

fn expect_error<T>(
    label: &str,
    result: Result<T, ai_tools::core::error::McpError>,
) -> ai_tools::core::error::McpError {
    match result {
...
fn run() {
    ...
    let terminal_relative = expect_ok(
        ...
        ai_tools::core::terminal_policy::resolve_contained_cwd(root, Some("src")),
    );
    assert_eq!(terminal_relative, canonical_root.join("src"));
    let terminal_escape = expect_error(
        "terminal traversal compatibility",
        ai_tools::core::terminal_policy::resolve_contained_cwd(root, Some("../external")),
    );
    assert_eq!(
