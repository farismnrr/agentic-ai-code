use ai_tools::application::lsp::{LspSessionManager, TypeScriptLanguageServer};
...
"#;
    ...
    // answer anything). `ai_tools::application::lsp` now answers that bridge
...
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<Output = Result<T, ai_tools::application::lsp::LspError>>,
    P: Fn(&T) -> bool,
{
