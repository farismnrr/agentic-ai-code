...
use ai_tools::interfaces::mcp::{find_tool, ToolCallResult};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let hooks = ai_tools::application::hooks::HookManager::load(Arc::new(config.clone())).unwrap();
    // -------------------------------------------------------------
    ...
    let task_id = ai_tools::application::execution::start_terminal_job(
    ...
    let snapshot = loop {
        ...
        if matches!(
            snapshot.state,
            ai_tools::application::execution::JobState::Completed
                | ai_tools::application::execution::JobState::Failed
        ) {
            break snapshot;
        }
