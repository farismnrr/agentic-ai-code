...
impl super::ServerConfig {
    ...
    pub fn agent_auth_roots_for(&self, provider: &str) -> Vec<std::path::PathBuf> {
        ...
        let candidates = match provider {
            ...
            "external-mcp" if home.join(".external-mcp/auth.json").is_file() => {
                vec![home.join(".external-mcp")]
            }
            ...
            "external-mcp" if home.join(".external-mcp").is_dir() => vec![home.join(".external-mcp")],
...
fn valid_provider(value: &str) -> bool {
    matches!(value, "external-mcp" | "agy" | "external-mcp")
}
