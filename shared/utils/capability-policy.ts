...
export function capabilityFactsForToolCall({
  ...
  const malformedInput = typeof input !== 'object' || input === null || Array.isArray(input)
    ...
    || (toolName === 'telegram_send_message' && !hasRequiredStrings(values, ['working_directory', 'message']))
  ...
  return {
    ...
    command,
    args,
    networkRequested: toolName === 'http_fetch' || toolName === 'web_search' || toolName === 'telegram_send_message' || toolName.startsWith('git_remote_') || toolName === 'git_fetch' || toolName === 'git_push' || toolName.startsWith('change_request_') || toolName.startsWith('issue_') || toolName.startsWith('workflow_')
      ? true
      : undefined,
...
export function toolEffects(toolName: string, annotations?: CapabilityAnnotations, trustedProvenance: CapabilityFacts['trustedProvenance'] = 'external'): CapabilityEffect[] {
  ...
  if (toolName === 'telegram_send_message') return ['network_write', 'external_mutation']
