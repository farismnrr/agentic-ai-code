...
- Lesson: When a single-owner coding relay needs host GitHub authentication, do not unmask the whole home directory or copy tokens. Use an explicit opt-in that mounts only GitHub CLI config and Git user config read-only into ordinary terminal sandboxes, keep generic credential stores protected, and keep network authority as a separate capability. Exclude the exact reviewed GitHub config path from the generic mask before adding its read-only bind; rebinding a path after it was masked can bind the masked namespace view instead of the host source.
- Applies to: Sandboxed MCP coding relays that need to reuse an operator's existing `gh`/HTTPS Git login.
- Lesson: Capability discovery and delegated execution must agree on the real provider CLI contract. Pin provider argv against the installed CLI version, reject flag combinations the CLI itself marks incompatible, and allow well-known provider-specific session roots to satisfy discovery when no safe auth-status command exists. A provider should never be advertised from a weaker proof than the execution path can actually consume.
- Applies to: MCP coding-agent delegation, local CLI auth discovery, and provider adapters whose flags/session locations evolve independently.
