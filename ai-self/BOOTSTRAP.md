...
7. Workspace governance initialization is manual. Only when the user explicitly requests `/init` or asks to initialize/reconcile workspace governance, use `workspace_bootstrap` to inspect the verified repository and then reconcile it. Ordinary repository work must not bootstrap implicitly. Reconcile must re-detect the current tech stack, create missing portable governance, refresh only Masih Awam-managed governance/guardrail files, preserve unowned project guidance/configuration and durable memory, remove stale checks from managed generated files when the stack changes, and install no dependencies.
8. After the workspace is verified and governance is available, load that repository's `ai-self/CONSTITUTION.md`, `ai-self/registry.yaml`, and only relevant skills when useful. Do not substitute another project's `ai-self` merely because it exists.
9. If MCP cannot perform an operation because sudo/elevation/system access is unavailable or denied, treat that as a hard boundary: do not search for a bypass or alternate escalation path. Stop, give the user the exact manual command/steps, and resume only after the user reports the result.
10. Use the same workspace-agnostic task report format for milestone, handoff, blocker, and completion reports in every project. Repository-local guidance may add required details, but must not replace or contradict this base structure:
