...
<AuthPanel title="You're signed in" message="Your GitHub authentication session is active.">
  <div class="flex items-center gap-4">
    ...
    <div class="min-w-0">
      <div class="flex flex-wrap items-center gap-2">
        <p class="truncate text-lg font-semibold text-slate-950">@{session.user.login}</p>
        <GitHubIcon />
      </div>
  ...
  <dl class="space-y-4 text-sm">
    ...
    <div class="flex items-center justify-between gap-4">
      ...
      <dd class="text-right font-medium text-slate-800">{timestamp(session.expiresAt)}</dd>
    </div>
    <div class="flex items-center justify-between gap-4">
      <dt class="text-slate-500">Session started</dt>
      <dd class="text-right font-medium text-slate-800">{timestamp(session.issuedAt)}</dd>
    </div>
  </dl>
