...
export default defineOAuthGoogleEventHandler({
  ...
  async onSuccess(event, { user: googleUser }) {
    ...
    if (existingUser) {
      ...
      if (!isEmailVerified) {
        throw createError({ statusCode: 403, message: 'Please verify your Google email address first before linking.' })
      }
    ...
    if (!isEmailVerified) {
      throw createError({ statusCode: 403, message: 'Google email must be verified to create an account.' })
    }
