...
export interface ChatConversation { id: string, modelId: string, workspaceId: string | null, mode: 'chat' | 'agent', permissionMode: 'plan' | 'bypass' | 'manual', enabledToolIds: string[], approvals: Record<string, string>, reasoningEffort: string | null, contextSummary: string | null, contextSummaryUpToMessageId: string | null, lastMeasuredTokens: number | null, lastMeasuredMessageId: string | null }
...
export interface SubagentToolInput { userId: string, parentSessionId: string, authority: SubagentAuthority, model: ChatModelHandle, enabledToolIds: string[], approvals: Record<string, string>, permissionMode: ChatConversation['permissionMode'], abortSignal: AbortSignal, taskNotifications?: TaskCompletionNotificationPort }
