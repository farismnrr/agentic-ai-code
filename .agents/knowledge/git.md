...
- `.external-mcp/settings.json` — shared hooks and settings for everyone on the repo.
- `.external-mcp/skills/*` — symlinks into `.agents/skills/`; git stores the link, not a copy.
- `.mcp.json`, `.env.example`, `skills-lock.json`.

Never committed — see `.gitignore`:

- `.env` and any `.env.*` other than the example.
- `.external-mcp/settings.local.json` and `.external-mcp/.credentials.json` — personal and machine-specific.
