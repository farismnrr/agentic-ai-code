...
- Plan 039C adds a narrow, vendor-neutral LSP substrate under `packages/rust-tools/application/src/lsp/` (session/process lifecycle in `manager.rs`/`protocol.rs`/`document.rs`; language-agnostic symbol/definition/reference/hover/diagnostic/rename normalization shared by every adapter in `semantic.rs` + `rename.rs`; thin per-language wrappers in `rust.rs` and `typescript.rs` covering rust-analyzer and typescript-language-server/`@vue/language-server`). It stays outside MCP transport routing; `packages/rust-tools/application/src/code.rs` is the only place that adapts it into the public `code_symbols`/`code_definition`/`code_references`/`code_implementations`/`code_hover`/`code_diagnostics`/`code_rename_preview` MCP tools (all read-only/preview-only — rename is normalize-and-return-only, never auto-applied; use `apply_patch`/`file_edit` to actually apply it). Do not add a second language-server framework, a repository-wide semantic index, or per-tool bespoke session management — extend `semantic.rs`/add a new thin adapter instead. `.vue` semantics are capability-gated: the currently-installed `@vue/language-server` needs a companion `@vue/typescript-plugin`-equipped `typescript-language-server` (hybrid mode) that is not installed; installing it is an explicit operator decision, never silent.
...
- `docs/` — human/operator installation, deployment, external MCP client, development, and release handbook.
