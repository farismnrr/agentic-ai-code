...
- **API responses are shaped server-side, fully — joins, pagination, filtering, all of it.** A screen that needs data from more than one table gets one endpoint that returns it pre-joined, not two endpoints fetched and merged client-side. Chaining/orchestrating multiple composable calls to assemble one screen repeatedly broke Nuxt's SSR composable context in this codebase; see the canonical memory's [Nuxt/application invariants](../memories/README.md#nuxtapplication-invariants). If FE code is about to combine, re-key, or sequence multiple API responses for one screen, strongly prefer moving that shape to the backend endpoint.
