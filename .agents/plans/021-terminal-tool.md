...
Researched and adopted from how other coding assistants (external MCP client Code, OpenHands, Aider) built this, and from a real disclosed vulnerability class in external MCP client Code's own Bash tool (CWE-78 command injection via shell-interpolated strings):
