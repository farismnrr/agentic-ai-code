...
- This index is the current plan-status source of truth for discoverability.
- New/active plans should keep their own top-level `Status` field aligned with this index. Some older plans predate standardized status metadata; classify those from current repository evidence and later closeout memories rather than silently treating an old unchecked verification box as an active plan.
...
- Every Markdown plan/sub-plan in this directory except this README must appear below. Supporting phase plans stay indexed even when their parent plan is complete.

## In Flight

- [029b-external-mcp-mcp-production-hardening.md](029b-external-mcp-mcp-production-hardening.md) — **IN FLIGHT.** Follow-up to Plan 029 for the remaining production/live-acceptance gaps. Repository hardening is substantially implemented, but live external MCP client/OAuth acceptance remains an explicit gate and Docker remains intentionally deferred until an isolated execution backend exists. The plan's branch-lock instructions remain historical/operational context for its implementation stream.

## Completed

- [029-external-mcp-native-mcp-integration.md](029-external-mcp-native-mcp-integration.md) — **COMPLETED for repository gates; live external MCP client/OAuth acceptance was deferred into Plan 029b.** Defines the stateless MCP `POST /mcp` target, OAuth resource-server model, `relay.coding` capability, frozen tool contract, deterministic conformance/security gates, and external MCP client integration requirements.
- [028-relay-agent-rust-rewrite.md](028-relay-agent-rust-rewrite.md) — **COMPLETED.** Replaced the legacy Node/WebSocket relay with the standalone Rust MCP relay and established the Bubblewrap filesystem/process boundary, non-root execution, OAuth/JWKS remote security model, deterministic security gates, and native release path. Some intermediate phase labels/checklists in the plan are historical snapshots from before final closeout; the plan header and final closeout evidence are authoritative for current status.
  - [028-phase12-legacy-relay-removal.md](028-phase12-legacy-relay-removal.md) — **COMPLETED supporting phase.** Removed the legacy relay compatibility surface so MCP is the sole current relay execution protocol.
  - [028-phase19-final-security.md](028-phase19-final-security.md) — **COMPLETED supporting phase.** Final filesystem/privilege/process/OAuth/CI/release security gates are recorded as complete; its previously stale status header has been reconciled.
- [027-cli-rust-refactor.md](027-cli-rust-refactor.md) — **COMPLETED.** Migrated terminal-tool, curl-tool, and searxng-search-tool executable CLIs from JavaScript to Rust; full parity, SSRF/security hardening, deterministic test suites, release CI pipeline, benchmarks, and zero-JS CLI cutover. PR #99.
- [026-local-cli-relay-agent.md](026-local-cli-relay-agent.md) — **COMPLETED HISTORICAL / SUPERSEDED.** First-generation browser-to-localhost relay plan. Its Node/WebSocket/no-jail design was later superseded by completed Plan 028; do not use Plan 026 as the current relay architecture reference.
...
- [022-otel-telemetry.md](022-otel-telemetry.md) — **COMPLETED/HISTORICAL.** Introduced the OpenTelemetry/Loki/Jaeger telemetry direction and containerized observability integration; current repository contains the telemetry server/plugin/API surfaces and later work built on them. Exact current deployment config must be read from source/config rather than this original design plan.
- [021-terminal-tool.md](021-terminal-tool.md) — **COMPLETED/HISTORICAL.** Introduced the terminal tool and chat/agent integration. Its original JavaScript CLI/package execution details were later superseded by Plan 027/028; use the current Rust tool/relay docs for executable security behavior.
- [020-tools-as-local-packages.md](020-tools-as-local-packages.md) — **COMPLETED/HISTORICAL.** Established local workspace packages and skills for curl/search. Its original `bin/cli.mjs`/`npx` CLI design was later superseded by Plan 027; TypeScript package APIs remain, standalone CLI execution is Rust.
- [019-search-mention-trigger.md](019-search-mention-trigger.md) — **COMPLETED/HISTORICAL.** Added explicit `@search`; the original provider-level forced-tool design proved unreliable and the shipped implementation directly invokes SearXNG then emits compatible tool UI chunks. See `../memories/019-search-forced-tool-choice-unreliable.md`.
- [018-chat-mode-langgraph.md](018-chat-mode-langgraph.md) — **COMPLETED/HISTORICAL.** Established chat-mode LangGraph/LangChain orchestration and the curl/search tool path that later plans extend. Current implementation lives under `server/utils/langgraph-*`; later memories supersede details that changed.
...
- [011-chat-prompt-sticky-footer.md](011-chat-prompt-sticky-footer.md) — **COMPLETED/HISTORICAL.** Diagnosed the dashboard-panel minimum-height/footer clipping issue; the current `app/app.config.ts` carries the resulting `dashboardPanel.slots.root: 'min-h-0'` override and rationale.
- [010-workspace-folders.md](010-workspace-folders.md) — workspaces become real folders under an operator-configured root instead of just names, aligning the app with local coding-assistant workspace semantics without adopting unrestricted filesystem browsing.
