...
- ChatGPT/third-party clients may impose request/proxy/UI time limits outside this repository, so server correctness cannot depend on holding one request open indefinitely.
...
- verify the exact MCP protocol revision implemented by this repository and current ChatGPT-compatible behavior;
...
11. ChatGPT live invocation of a task-eligible operation does not remain indefinitely stuck when the client supports the negotiated lifecycle; otherwise fallback behavior is documented truthfully;
...
- deployed ChatGPT and first-party acceptance cover timeout/disconnect/cancel/retrieve paths;
