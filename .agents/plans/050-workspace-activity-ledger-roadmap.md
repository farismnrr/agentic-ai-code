...
Deliver an industry-standard, persistent, per-workspace activity ledger that truthfully records every workspace-scoped operation mediated by `ai-tools relay`, regardless of whether the caller is the first-party Nuxt application, the paired/local-terminal path, ChatGPT through remote MCP, another MCP client, or a task/background execution path.
...
2. first-party Nuxt MCP, paired/local-terminal calls, direct ChatGPT remote MCP, generic MCP clients, and sync/async task/job execution all use the same activity contract;
...
If identifying metadata is absent or untrusted, the UI shows **External MCP client** rather than guessing. A `ChatGPT` label is used only when actual reviewed metadata supports it.
...
### Direct external MCP / ChatGPT
When deployment/connector is available and separately authorized, execute harmless read/search and optional controlled structured mutation. Confirm the operation appears without Nuxt being the caller. Inspect actual `clientInfo`: show `ChatGPT` only when real metadata supports it; otherwise display `External MCP client` and mark exact actor identification UNPROVEN.
