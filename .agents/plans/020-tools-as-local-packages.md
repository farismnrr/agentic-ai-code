...
5. Symlink `.agents/skills/curl-tool -> ../../packages/curl-tool` and `.agents/skills/searxng-search-tool -> ../../packages/searxng-search-tool` (each package's `SKILL.md` becomes discoverable exactly like an installed skill, mirroring `.external-mcp/skills -> .agents/skills`).
