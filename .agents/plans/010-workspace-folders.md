...
Plan 007 introduced "workspaces" purely as a named grouping for chats — a `text` name in Postgres, no tie to the filesystem at all. That doesn't match how the local coding assistants this app was modeled after actually work. Confirmed by research, not assumption:

- **Local coding assistants/CLIs** — single-operator tools. "Workspace" is typically the directory the tool is launched from (or receives as an argument); access is bounded by the operator's OS user permissions, not by a multi-user application policy. They do not solve a multi-user server-side directory picker because they do not have the same multi-user trust model.
- **Local web wrappers for coding assistants** — generally expose one operator's filesystem through a localhost/LAN service, still without per-account filesystem scoping.
...
- ✅ A security review was run against the merged diff (`f4ac161..a1347f8`) as a fast-follow (the plan required it before merging, but the branch had already been merged before this pass ran — see `.agents/knowledge/git.md` process gap noted below). It found one real bug and one already-accepted tradeoff:
...
**Process note**: this plan's own "Verification" section required a security review *before* merging, but the branch was pushed, reviewed, and merged without it — caught only when re-reading `.agents/knowledge/` after the fact. The symlink bug above is a direct consequence: it's exactly the class of issue that review step exists to catch. Run the security review before opening the PR next time, not after merging.
