...
- The relay formats the final message from structured fields so both Nuxt-originated and ChatGPT-originated completion signals have the same report shape.
...
- [x] Update completion signal guidance so ChatGPT supplies a useful report covering changes, validation, and remaining risks when applicable.
