...
2. **Narrowed command surface.** Read the installed `@vue/language-server@3.3.8`'s `lib/server.js` (every `sendTsServerRequest(...)` call site) and the bundled `@vue/typescript-plugin@3.3.8`'s `index.js` (every `session.addProtocolHandler('_vue:...', ...)` registration) — the two lists match exactly, 17 commands. Only that closed, explicit allowlist (`tsserver_bridge/commands.rs::ALLOWED_COMMANDS`) is ever forwarded, with a positional-vs-object argument-shape table (`POSITIONAL_FILE_COMMANDS`) used to extract and validate any file-bearing argument before forwarding. Unknown/unlisted commands are answered with bounded `null` and never forwarded.
3. **Real bounded stdout framing.** Replaced the `read_line(String)` (which only checked the 1 MiB limit after allocation) with an incremental `fill_buf`/`consume`-based `read_bounded_line` (`tsserver_bridge/framing.rs`) that rejects before any oversized accumulation. Any read error, oversized line, or mid-line EOF now faults the bridge closed rather than merely returning from the loop.
4. **Bound work before spawning.** `handle_tsserver_request` now acquires a bounded `tokio::sync::Semaphore` permit (`TsServerBridge::try_acquire`, sized to `MAX_TSSERVER_PENDING`) *before* spawning any forwarding task; at capacity it spawns only the trivial "reply null" task, never the heavy forwarding work.
...
6. **Fail closed on bridge death.** `TsServerBridge` now shares an `Arc<AtomicBool>` fault flag with the owning session's `SharedState::faulted` (previously bridge-only, never touched the session). Any fatal condition — oversized/malformed child output, stdin/stdout write failure, or clean/unclean child exit — calls `mark_fatal()`, which sets that shared flag, drains outstanding requests with `null`, and reaps the child. `LspSession::is_faulted()` and `LspSessionManager::session_for` therefore treat the whole Vue session as unhealthy and replace it on next use (preserving restart budgets), exactly like a primary-child crash, instead of the bridge silently degrading to `null` forever while the session kept reporting healthy.
...
Re-verified real Vue `<script setup lang="ts">` semantics against the installed `@vue/language-server@3.3.8` / `@vue/typescript-plugin@3.3.8` pair after the fix: document symbols remain real and TypeScript-backed (proven, unchanged). Definition/references/hover/diagnostics remain bounded, public-safe *empty* results, not errors/hangs/crashes — none of the six bridge findings were the cause of that emptiness (they are security/framing/lifecycle boundary defects, not semantic-resolution logic), which matches the PHASE-04 finding already independently reproduced outside this crate with a minimal Node harness driving the same installed binaries directly. This is a genuine limitation of the installed `@vue/language-server@3.3.8` build's local embedded TypeScript language service for files only known via `textDocument/didOpen`, not a redefinition of the acceptance bar, and not something this remediation's scope (the bridge boundary) can fix.

Gates re-run and passing after this remediation: `scripts/verify-lsp-foundation.sh`, `rust_lsp_acceptance`, `typescript_lsp_acceptance` (TypeScript + Vue), `scripts/verify-code-mcp.sh`, `scripts/verify-rename-preview.sh`, `scripts/verify-post-edit-diagnostics.sh`, `scripts/phase-039c-contract.sh`, `scripts/phase7-chatgpt-contract.sh`, `scripts/verify-workspace-path-security.sh`, `scripts/phase8-zero-bypass.sh`, `scripts/check-architecture.sh`, `node scripts/check-maintainability.mjs`, and `pnpm verify:commit`.
