# Plan 029 — external MCP client Native MCP Integration

Status: IN FLIGHT

## Goal

Extend the Rust Relay Agent from the Plan 028 local/legacy-compatible MCP server into a production-grade **remote MCP server** that can be connected natively from external MCP client and also remain compatible with external MCP client remote MCP clients.

The primary goal is **not** to build a custom external MCP client-only protocol. The implementation should target the current MCP standard first, then add compatibility only where a real client requires it. As of the current MCP release (`2026-07-28`), the standard HTTP transport is stateless and uses a single MCP endpoint with POST/GET semantics; the older HTTP+SSE transport is deprecated. external MCP client currently supports remote MCP apps and full write/modify actions in developer mode for supported Business/Enterprise/Edu plans, and local servers must be reached through the supported secure tunnel rather than being exposed directly. citeturn1search1turn3search0

## Non-goals

- Do not build a bespoke OAuth Authorization Server in Rust unless a later plan explicitly requires it.
- Do not make external MCP client-specific behavior the core protocol implementation.
...
- Do not expose the relay directly to the public internet merely to make external MCP client connectivity work.
...
### B. external MCP client connectivity

external MCP client custom MCP apps connect to **remote MCP servers**. A relay running on a developer machine should therefore be exposed through the supported Secure MCP Tunnel instead of opening a raw public listener. Full MCP write/modify actions are currently available in beta for external MCP client Business and Enterprise/Edu, while availability and permissions may change. citeturn3search0
...
### D. external MCP client OAuth metadata

external MCP client expects an MCP app to provide endpoint/metadata information and, when OAuth is configured, the provider must support a refresh-token strategy. OpenAI specifically recommends verifying that the provider advertises and issues `offline_access` (or the provider-equivalent refresh capability), otherwise external MCP client may lose connectivity after access-token expiry. citeturn0search1

The exact external MCP client callback URL must be copied from the external MCP client app setup UI and registered exactly at the IdP; do not hardcode or invent a generic callback URL. citeturn0search3turn0search4
...
external MCP client itself can provide action controls and confirmations for write/modify actions, but these are client-side safety UX and must never replace server-side authorization or sandbox enforcement. citeturn3search0

### F. external MCP client compatibility

external MCP client Code supports remote MCP over HTTP and SSE and supports OAuth for remote MCP servers. Anthropic's MCP connector also supports remote HTTP MCP servers with OAuth bearer tokens. Therefore, the canonical implementation should target standard Streamable HTTP first while keeping a narrow compatibility strategy for clients that still require legacy SSE. citeturn2search0turn2search1
...
- [ ] Read OpenAI external MCP client custom MCP app/developer-mode requirements.
...
- [ ] Store implementation-specific decisions in `.agents/memories/029-external-mcp-mcp-integration-decisions.md`.
...
- [ ] Target external MCP client and external MCP client client compatibility matrix is documented.
...
Make the relay's MCP surface deterministic and safe for external MCP client tool scanning and external MCP client tool discovery.
...
- [ ] external MCP client tool scan discovers the intended tools with correct names/schemas.
- [ ] external MCP client remote MCP discovery sees the same intended tool contract.
...
Make external MCP client/external MCP client able to discover and complete the external OAuth flow without the relay becoming an Authorization Server.
...
- [ ] Configure exact external MCP client callback URI from the external MCP client UI; never guess it.
- [ ] Configure the corresponding external MCP client/Anthropic OAuth callback requirements where applicable.
...
- [ ] external MCP client can discover the authorization metadata.
- [ ] OAuth authorization succeeds.
- [ ] Token refresh succeeds without manual re-login.
- [ ] external MCP client remote MCP OAuth flow succeeds where supported by the chosen client/provider.
...
Make local development usable from external MCP client without turning the developer machine into a public unauthenticated command server.

### Tasks

- [ ] Document the supported Secure MCP Tunnel workflow for external MCP client local/developer-machine deployments.
...
# Phase 6 — external MCP client Native Integration E2E

### Objective

Prove that external MCP client can discover, authorize, and use the relay as a real custom MCP app.

### Tasks

- [ ] Create a external MCP client custom MCP app in developer mode using the actual endpoint and metadata.
...
- [ ] Verify destructive operations still require the intended external MCP client confirmation/permission behavior where applicable, while server-side authorization remains authoritative.
- [ ] Verify the relay's Plan 028 sandbox remains enforced during all external MCP client calls.
...
- [ ] Real external MCP client conversation can inspect, modify, build, and run code through the relay.
...
# Phase 7 — external MCP client Remote MCP Compatibility

### Objective

Use the same MCP server for external MCP client integrations without maintaining a second implementation.

### Tasks

- [ ] Connect external MCP client Code to the canonical HTTP MCP endpoint.
...
- [ ] Verify external MCP client cannot access terminal execution without the same required scope.
- [ ] Verify external MCP client can complete a realistic coding workflow.
- [ ] Verify no client-specific branch weakens security controls.

Anthropic documents remote HTTP and SSE MCP transports and OAuth support for remote MCP connections, so this phase should validate the shared server rather than build a external MCP client-specific API. citeturn2search0turn2search1

### Exit criteria

- [ ] external MCP client and external MCP client use the same canonical MCP tool implementation.
...
- [ ] An operator can diagnose failed external MCP client/external MCP client connections without accessing secrets.
...
- [ ] external MCP client native MCP connection works.
- [ ] external MCP client remote MCP connection works.
...
- [ ] Update `.agents/memories/029-external-mcp-mcp-integration-decisions.md` with final decisions and actual client behavior observed during E2E.
...
- OpenAI — Developer mode and MCP apps in external MCP client: current remote-MCP requirements, custom app lifecycle, OAuth/refresh-token considerations, write-action behavior, and Secure MCP Tunnel guidance. citeturn3search0
- OpenAI — Apps in external MCP client: custom MCP apps, app/plugin relationship, workspace administration, and publishing model. citeturn0search0
- OpenAI — external MCP client app templates: exact callback URL handling and provider configuration guidance. citeturn0search3turn0search4
...
- Anthropic — external MCP client Code MCP: remote HTTP/SSE transport and OAuth support. citeturn2search0
...
Plan 029 is complete only when the same Rust Relay Agent can be securely consumed as a remote MCP server by external MCP client and external MCP client, using the current MCP transport as the canonical path, with standards-compliant OAuth discovery/authorization, explicit scope/subject enforcement, the existing Plan 028 sandbox as the authoritative execution boundary, strict CI/release gates, and a successful real-world coding workflow. No client-specific compatibility path may weaken the security model.
