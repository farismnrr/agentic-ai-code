...
- [ ] Freeze a external MCP client/external MCP client compatibility matrix.
...
- [ ] Verify whether external MCP client's current scanner requires `GET /mcp`, POST SSE responses, or a specific `Accept` behavior; implement only requirements confirmed against the live client/current MCP docs.
...
- [ ] Verify `Content-Type` and `Accept` negotiation against external MCP client's real tool scanner.
...
# Phase 2 — external MCP client-Grade Tool Descriptors & Schema Stability

### Objective

Make existing tools safe, understandable, scope-aware, and stable when external MCP client freezes their definitions.
...
- [ ] Add external MCP client-compatible OAuth/security metadata or scope tags at the tool/action descriptor layer where the current OpenAI contract supports them.
...
- [ ] Treat removing/renaming required fields or tools as breaking changes requiring external MCP client action refresh/republication.
...
- [ ] external MCP client Scan Tools presents clear, stable actions.
- [ ] Tool schemas match actual server behavior and scope requirements.
- [ ] Future schema changes cannot silently break external MCP client's frozen snapshot without documented migration.
...
- [ ] Decide registration methods explicitly:
  ...
  - [ ] CIMD — advertise/support when current MCP + external MCP client integration path supports it.
...
- [ ] DCR/CIMD availability shown by external MCP client matches what the server/IdP actually advertises.
...
Make authentication durable and compatible with external MCP client's production connector lifecycle.
...
- [ ] Use OpenAI Secure MCP Tunnel for supported local/private-network external MCP client development when available/applicable.
...
- [ ] external MCP client can reach a developer-machine relay through the approved tunnel path.
- [ ] Closing the tunnel removes remote reachability.
- [ ] No unauthenticated remote fallback exists.

---

# Phase 7 — Real external MCP client Custom MCP App E2E

### Objective

Validate the exact current external MCP client UI and runtime, not only protocol-level mocks.
...
- [ ] Copy/register the exact external MCP client callback URL.
...
- [ ] Document that breaking action/tool changes require external MCP client action refresh/republication.

### Exit criteria

- [ ] A real external MCP client conversation can inspect, modify, build, and run code through the relay.
- [ ] No server-side security rule depends on external MCP client confirmation UX.

---

# Phase 8 — external MCP client Compatibility E2E
...
- [ ] Connect external MCP client remote MCP client to the canonical `/mcp` endpoint.
...
- [ ] Add legacy SSE adapter only if an actual supported external MCP client target cannot use the canonical transport.
- [ ] If legacy adapter is required, no separate auth/tool/sandbox implementation is allowed.

### Exit criteria

- [ ] external MCP client and external MCP client share exactly one MCP/tool/security core.
...
Prevent external MCP client compatibility work from weakening Plan 028 quality/security guarantees.
...
- [ ] Current external MCP client Custom MCP App connects successfully.
...
- [ ] external MCP client uses the same canonical server successfully.
...
- [ ] Breaking tool-schema changes have a documented external MCP client refresh/republication process.
...
- [ ] `.agents/memories/029-external-mcp-mcp-integration-decisions.md` records actual client behavior and final decisions.

---

## Source-of-truth references

Review these again at implementation time because external MCP client/MCP integration behavior is actively evolving:

- OpenAI — Developer mode and MCP apps in external MCP client: https://help.openai.com/en/articles/12584461-developer-mode-and-full-mcp-connectors-in-external-mcp
- OpenAI — Apps in external MCP client: https://help.openai.com/en/articles/11487775-connectors-in-external-mcp
