...
- [ ] Record exact current external MCP client custom-app OAuth UI fields.
- [ ] Re-check official OpenAI developer-mode/custom-MCP docs.
- [ ] Re-check current MCP authorization/client-registration docs.
- [ ] Freeze the external MCP client plan/mode used for write-capable E2E.
...
- [ ] Do not add external MCP client-specific execution code to handlers.
- [ ] Do not split `terminal_exec` into artificial micro-tools unless live external MCP client behavior proves it necessary.
...
- [ ] no external MCP client-specific route bypasses canonical auth/dispatch/sandbox core.
