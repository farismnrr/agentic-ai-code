...
11. **ChatGPT connector/action resync is operator-owned too.** Do not attempt to force/recreate/resync the user's ChatGPT MCP connection. If the MCP catalog/schema/runtime needs a fresh ChatGPT snapshot, provide the exact resync/reconnect steps only when the new runtime is ready for live proof.
...
implement child/phase work
  ...
  -> STOP and hand the operator ChatGPT connector/action resync steps if catalog/schema changed
  -> operator resyncs/reconnects ChatGPT
...
| --- | --- | --- |
| ChatGPT/MCP calls can appear stuck or time out around slow operations | **040A** | Separate short HTTP request lifetime from durable task lifetime; explicit deadlines, cancellation, retained result and reconnect/retrieval |
