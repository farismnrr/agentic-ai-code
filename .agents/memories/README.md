...
- **Plan 035 — `035-end-to-end-observability-and-secure-telemetry.md` is OPEN / REMEDIATION ROUND 3 IN PROGRESS through Phase 4, on branch `feat/035-p0-observability-contract`.** Round 1 and round 2 histories remain preserved. Durable reopening categories are: (a) raw exception confidentiality in Jaeger, (b) LangGraph/tool streamed-error confidentiality, (c) Rust telemetry dependency/filesystem-path confidentiality, and (d) Nuxt→`ai-tools` subprocess trace continuity; evidence gaps are browser reality, genuine Rust 5xx proof, requestId→Loki→trace guarantee, and closure-claim accuracy. A shared trace requires a real W3C or explicit subprocess parent/child handoff; request IDs, timestamps, or wall-clock correlation do not prove it. Relay HTTP and the first-party `ai-tools` subprocess are separate boundaries; third-party providers receive no internal propagation. No later round-3 phase is complete until independently re-proven.
  - **Phase 5 is currently blocked, not passed:** the request-lifecycle implementation has not been committed because the required live acceptance could not prove all four journeys from returned `x-request-id` through Loki to Jaeger. The available running services did not match the script's configured app/Loki endpoints, and the repository has no safe existing controlled route for a genuine 5xx journey without inventing an unsafe debug path or relying on unavailable auth/provider state. Do not describe Phase 5 or Plan 035 as complete until that live evidence exists.
