...
- [018-langgraph-ui-stream-bridge.md](018-langgraph-ui-stream-bridge.md) — real ai@7 writer API, valid dynamic-tool states, and the streamEvents v2/v3 tradeoff for the LangChain→UI-stream bridge
- [018-sidebar-single-fetch.md](018-sidebar-single-fetch.md) — chaining two client-side composable calls to assemble the sidebar kept breaking SSR context; BE now returns workspaces+conversations joined in one endpoint
...
- [029-external-mcp-mcp-integration-decisions.md](029-external-mcp-mcp-integration-decisions.md) — Plan 029 freezes the external MCP client MCP target around stateless `POST /mcp`, Auth0-backed OAuth, and `relay.coding`
