...
- `pnpm verify:commit` now also runs `scripts/check-architecture.sh` (Plan 031A finding I): `relay_agent::mcp` transport-independence, no H3 event objects in `server/application`/`server/infrastructure`, and no direct Drizzle schema/`drizzle-orm` or `@ai-sdk/*`/`@langchain/*` imports from `server/application/**`. Proven to fail on an injected violation and pass once reverted; extend it (or add ESLint `no-restricted-imports`) only for boundaries that actually exist in shipped folders.
- `scripts/check-agent-docs.sh`'s vendor-agent-path list is checked against **git tracking**, not raw filesystem existence — a coding agent's own untracked local runtime directory (worktrees, lock files) is expected to exist on disk without being repository-owned, and one of the listed paths is gitignored for the same reason. If this check starts failing again for a path that's genuinely untracked, that's the bug, not the path. (This note deliberately avoids spelling out the exact path/vendor name inline — see the script itself — because the agent-doc checker's own grep flags that literal wording.)
- A `nuxt prepare` in this project's sandboxed execution environment has been observed to emit only `.nuxt/tsconfig.server.json`, never `.nuxt/tsconfig.json`/`tsconfig.app.json` — reproduces identically on a clean, unmodified `dev` checkout, so it is an environment limitation, not a source defect. `vue-tsc -p .nuxt/tsconfig.json --noEmit` and `pnpm build` fail there as a result. Substitute `vue-tsc -p .nuxt/tsconfig.server.json --noEmit` for a partial (server-only) type-check in that environment, but treat the full Vue/client typecheck and `pnpm build` as **not yet proven** until run somewhere `nuxt prepare` emits the complete tsconfig set.
...
- **Open, non-blocking finding (discovered during Plan 031A closure, not yet fixed):** `scripts/phase4-black-box.sh`'s malformed-bearer-token case gets HTTP 500 instead of the expected 401. The request is still rejected either way (fail-closed holds, no tool dispatch occurs) — this is a status-code precision bug, not an authorization bypass. Root cause traced to `transport.rs`'s JWKS-cache-refresh branch returning `INTERNAL_SERVER_ERROR` on any refresh failure *before* the token itself is parsed, rather than the token-parsing path itself (which does correctly 401 on a malformed token). Reproduces identically on commits well before Plan 031/031A. Needs a real fix, not a plan-closure rush job.
- **Open, non-blocking finding (discovered during Plan 031A closure, not yet fixed):** `scripts/phase7-chatgpt-contract.sh` fails immediately — it reads an expected tool-catalog SHA256 from `.agents/memories/029-phase7-published-app-lifecycle.md`, which no longer exists after the 2026-08-12 memory compaction into this single file. The recorded hash value itself appears to have been lost in that compaction, not just moved; regenerating it from the current `.agents/contracts/029-tool-catalog-v1.json` would make the check circular (no independent tamper evidence), so this needs a deliberate decision about where the tamper-evidence hash should live now, not a mechanical path fix.
