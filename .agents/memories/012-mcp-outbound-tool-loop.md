...
**`/security-review` finding, fixed in the same PR — `mcpServers.url` is an SSRF vector.** It's a bare user-supplied string with no upstream validation; `createMcpClient` opens a real server-side connection to it. Any authenticated user could point a server at `169.254.169.254` (cloud metadata) or an internal address and trigger the connection via the "test connection" endpoint or by enabling the tool in chat. Fixed with `assertSafeMcpUrl()` in `server/utils/mcp-client.ts`: http(s)-only, DNS-resolved and checked against loopback/RFC1918/link-local/metadata ranges, re-checked on every connect (not just at server registration) to also cover DNS rebinding. **If a future change adds a new way to reach `createMcpClient` (or a similar connect-to-user-url path elsewhere), it inherits this guard automatically only if it goes through `createMcpClient` — don't add a second connector that bypasses it.**
