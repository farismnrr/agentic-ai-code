...
`scripts/phase6-external-mcp-e2e.sh`. It verifies the local MCP contract and Plan
...
| --- | --- | --- |
| Tool discovery | tool catalog and required MCP headers are present | external MCP client Scan Tools must be run in the configured workspace |
...
| Coding workflow | terminal, HTTP, search, and Plan 028 execution/security surfaces are present | inspect/edit/install/build/run/verify must be executed in external MCP client |
...
No live external MCP client session, OAuth tenant, callback, or deployment URL is
