...
Reasoning:
- **Security & Multi-Tenancy**: The app is architecturally a multi-tenant web server (Postgres-backed accounts, login). Allowing an authenticated web user to browse the unrestricted server filesystem is inherently dangerous. None of the local-only reference tools (external MCP client Code, Antigravity CLI) solve multi-user filesystem access because they run under the local operator's OS permissions.
