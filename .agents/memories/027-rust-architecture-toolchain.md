...
- `std::net::ToSocketAddrs`: Used by `curl-tool` to pre-resolve DNS natively and enforce strict SSRF blocks (e.g., rejecting DNS rebinds to 127.0.0.1) without relying on external dependencies like `trust-dns-resolver` or `hickory-resolver`.
