...
| Public deployment acceptance | Old ChatGPT acceptance script referenced pre-layered Rust paths and only made a weak metadata probe | Current layered paths + resource-path Protected Resource Metadata validation | Repair the script and ensure it never labels a metadata-only probe as a full ChatGPT E2E success. |
...
6. **Live evidence:** neither a static source check nor a Protected Resource Metadata curl is sufficient to claim ChatGPT interoperability. Final acceptance requires a real ChatGPT developer-mode connection, OAuth completion, tool discovery, and tool call.
