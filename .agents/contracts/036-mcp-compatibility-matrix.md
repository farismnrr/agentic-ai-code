...
| Authorization Server | Not implemented in repo by design | Auth0 reference profile, with the relay remaining generic OAuth/JWKS | Configure a real Auth0 tenant/API for the exact MCP resource, `relay.coding`, owner subject, PKCE, and a ChatGPT-supported client-identification mode; do not write an OAuth issuer inside the relay. |
...
8. **Reference Authorization Server:** Auth0 is the concrete IdP profile because current OpenAI MCP guidance explicitly supports it and recommends an established IdP. The Rust relay remains provider-neutral and validates ordinary issuer/audience/JWKS/owner/scope claims.
9. **Live evidence:** neither a static source check nor a Protected Resource Metadata curl is sufficient to claim ChatGPT interoperability. Final acceptance requires a real ChatGPT developer-mode connection, OAuth completion, tool discovery, and tool call.

## External facts rechecked

At implementation start, official MCP documentation identifies `2026-07-28` as the modern stateless HTTP revision, requires OAuth Protected Resource Metadata for protected HTTP resources, treats the MCP server as the Resource Server, and prefers Client ID Metadata Documents while retaining compatibility mechanisms such as DCR where supported. Current OpenAI MCP documentation requires a stable public HTTPS MCP endpoint for production integrations and supports OAuth 2.1/PKCE with MCP resource binding for authenticated ChatGPT connections. Current OpenAI guidance recommends an established identity provider and explicitly documents Auth0 as an MCP-compatible option. Current Cloudflare Tunnel documentation describes an outbound-only tunnel that maps a public hostname to a local HTTP service and forwards the visitor protocol through `X-Forwarded-Proto`.
