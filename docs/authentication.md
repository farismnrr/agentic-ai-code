...
## external MCP client authentication

external MCP client does not use the Nuxt application's private relay token. It connects directly to the public MCP resource and completes its own OAuth flow with the Authorization Server.

That means a valid external MCP client setup requires all three pieces to agree on the same resource contract:

```text
external MCP client OAuth client
      -> Authorization Server
      -> access token for https://mcp.example.com/mcp
      -> ai-tools relay validates token
```

Continue with [keycloak.md](keycloak.md), [remote-mcp.md](remote-mcp.md), and [external-mcp.md](external-mcp.md).
