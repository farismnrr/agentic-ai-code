...
In current Keycloak terminology, create/select the client scope used for MCP, add an **Audience** protocol mapper, put the exact MCP URL in **Included Custom Audience**, and enable **Add to access token**. Then link that scope to the ChatGPT/MCP client as default or optional according to your policy.
...
Interactive clients such as ChatGPT must use an authorization-code flow with PKCE. Configure Keycloak/client policy so public/interactive MCP clients can use PKCE S256 and do not require exposing a static client secret inside ChatGPT.
...
ChatGPT's exact supported client-registration UI can evolve, so use the mode supported by the current ChatGPT connection flow:
...
- restrict trusted redirect/callback hosts to the current ChatGPT hosts shown by the live connection UI;
- reject arbitrary/untrusted redirect origins;
- do not guess a callback URI from an old session and permanently allowlist it.

Use the **exact callback URI displayed by ChatGPT when creating the connection**.

### Refresh-token support for ChatGPT

For persistent ChatGPT connectivity, configure Keycloak so the client can obtain refresh tokens when ChatGPT requests them. OpenAI's current OIDC guidance recommends `offline_access` be supported/advertised for this purpose. Keep `relay.coding` as the relay permission scope; `offline_access` is a client-session/token-lifecycle capability.

## 8. Verify Keycloak before starting ChatGPT
