...
The first-party chat UI presents those same capability facts through category-driven tool cards and bounded approval summaries rather than raw tool JSON. Diff/result previews are capped, subagent/background/orchestration state is compact, and hidden reasoning is never required for progress UX. Multi-agent orchestration is a parent-owned bounded DAG layered on the existing subagent/background runtime: dependency-ready nodes are role-routed through profile/authority intersection, writer nodes require isolated worktrees, stale child leases are rejected, and cancellation propagates to owned child processes. A separate bounded reconciliation ledger deduplicates evidence, surfaces reviewer disagreement and P0/P1 blockers, and distinguishes produced/reviewed/accepted/integrated/delivered writer states; it never performs Git delivery itself, so Plan-040 Git/forge primitives remain the only delivery path. Action observability reuses the Plan-035 OpenTelemetry/logger pipeline and its single allowlist sanitizer; semantic attributes may include stable tool/category/effect/policy/result/agent/orchestration state and bounded counts, but never tool arguments/results, prompts, credentials, private absolute paths, or raw provider/tool errors. The relay remains the hard authority even when the UI shows an approval or risk classification.
