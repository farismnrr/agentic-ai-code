...
if [[ -n "${AI_CODE_GUARD_BASE_SHA:-}" ]]; then mapfile -t changed < <(git diff --name-only --diff-filter=ACMRD "$AI_CODE_GUARD_BASE_SHA" "$AI_CODE_GUARD_HEAD_SHA"); else mapfile -t changed < <({ git diff --name-only --diff-filter=ACMRD; git diff --cached --name-only --diff-filter=ACMRD; git ls-files --others --exclude-standard; } | sort -u); fi
