...
test "$(jq -r '.[].name' "$catalog" | paste -sd' ' -)" = "terminal_exec http_fetch web_search directory_list file_search text_search terminal_job_start terminal_job_get terminal_job_cancel"
test "$(jq -r '.[].title' "$catalog" | paste -sd' ' -)" = "Sandboxed Coding Terminal HTTP Fetch Web Search Directory List File Search Text Search Start Terminal Job Get Terminal Job Cancel Terminal Job"
