...
# Phase 6 acceptance harness. Static checks always run; live external MCP client evidence is
# opt-in because this repository cannot drive a user's external MCP client workspace.
...
if [[ -n "${PHASE6_MCP_URL:-}" ]]; then
  ...
  echo 'LIVE_EXTERNAL-MCP_E2E=unavailable (set PHASE6_MCP_URL for deployment metadata probe)' >&2
fi
