...
NUXT_OTEL_ENABLED=true NUXT_OTEL_JAEGER_ENDPOINT="$NUXT_OTEL_JAEGER_ENDPOINT" \
...
curl -sS "http://127.0.0.1:$port/mcp" "${headers[@]}" --data '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"terminal_exec","arguments":{"command":"printf phase3-canary"}}}' >/dev/null

kill "$pid" 2>/dev/null || true
wait "$pid" 2>/dev/null || true
pid=''
sleep 1
payload=$(curl -fsS --get "$JAEGER_QUERY_URL/api/traces" --data-urlencode 'service=ai-code-relay' --data-urlencode 'limit=100')
grep -q 'relay.request' <<<"$payload"
grep -q 'relay.mcp.initialize' <<<"$payload"
grep -q 'relay.mcp.tools_list' <<<"$payload"
! grep -Eq 'h2::|hyper::|tonic::|reqwest::|/\.cargo/registry|/packages/rust-tools/|phase3-canary' <<<"$payload"
